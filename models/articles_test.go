...
var (
	articleDBTypes = map[string]string{`ID`: `bigint`, `Title`: `character varying`, `Content`: `text`, `CreatedAt`: `timestamp without time zone`, `UpdatedAt`: `timestamp without time zone`, `DeletedAt`: `timestamp without time zone`}
	_              = bytes.MinRead
)
